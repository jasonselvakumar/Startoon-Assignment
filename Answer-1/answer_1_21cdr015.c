...
void load_data(const char *filename, double *data, int *size) {
    ...
    *size = 0;
    while (*size < MAX_DATA_SIZE && fscanf(file, "%lf", &data[(*size)++]) != EOF);
    
    if (*size == MAX_DATA_SIZE) {
        fprintf(stderr, "Warning: Data truncated to fit MAX_DATA_SIZE.\n");
    }
    
    fclose(file);
}

void find_peaks(const double *data, int size, int *peaks, int *num_peaks, int distance, double prominence) {
    *num_peaks = 0;
...
void plot_ascii(const double *data, int size, const int *peaks, int num_peaks, const int *minima, int num_minima, const char *label) {
...
void find_and_plot_peaks(const double *data, int size, const char *label) {
